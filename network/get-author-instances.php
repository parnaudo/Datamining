...
$authorID=1;
$filter="(Rhuematoid Arthritis [MESH FIELDS] OR Rhuematoid Arthritis [Title] OR Rhuematoid Arthritis [Journal])";
...
//if there are papers, insert an author record	
	...
	foreach($uids['papers'] as $key=>$value){
		$paperQuery="SELECT * FROM papers where id=".$value[0];
		$paperTest=mysql_num_rows(mysql_fetch_array(mysql_query($paperQuery)));
		...
		//Get the rest of the paper info
		$valueArray=array(
