        ...
        while($row=mysql_fetch_array($result)){
                ...
                while($authorRow=mysql_fetch_array($authorResult)){
                 ...
                 if(sizeof($testArray)==0){
                        //if we found no records, best check to make sure we didn't miss anything in the reject list
                        $initials='';
                        $matchKey='';


                        uasort($rejectArray,'lengthsort');
                        foreach($rejectArray as $key=>$value){
                                echo "$value VS ".$infoRow['firstName']."<BR>";
                                if(stripos($value,$infoRow['firstName'])!==FALSE && $matchKey==''){
                                        $updateAuthor="UPDATE authors set atomId=".$infoRow['atomId']." where id=".$key;
                                        $updateInstance="UPDATE coAuthorInstance SET query='".$value."' where coAuthor=".$key ;
                                        echo $updateAuthor." BREAK<BR>";
                                        echo $updateInstance;
                                        mysql_query($updateAuthor);
                                       mysql_query($updateInstance);
                                        $chars = preg_split('/ /', $value, -1, PREG_SPLIT_OFFSET_CAPTURE);
                                                
                                                foreach($chars as $char){
                                                        
                                                        $initials=$initials." ".substr($char[0],0,1);
                                                }
                                                $matchKey=$key;
                                }
                                else{
                                        //check if the initials match up with the person we just found
                                        echo "INITIALS $value vs $initials <BR>";
                                        if(stripos(trim($value), trim($initials))!==FALSE){
                                                $updateInstance="UPDATE coAuthorInstance SET coAuthor=".$matchKey.", query='".$value."' where coAuthor=".$key;
                                                mysql_query($updateInstance);
                                                echo $updateInstance;
                                        }
                                }
